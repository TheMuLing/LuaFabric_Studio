...
- 侧边栏改造：
  ...
  - 分割线上方改为横向登录卡：圆形头像 + 「立即登录」居左、MD3「签到」按钮居右，点击整卡带波纹（覆盖分割线以上区域）

  - 新增「源码论坛」功能项（forum 图标，页面暂空置）

  - Debug 控件移至最底部，与「LuaFabric Studio」版权行同排右对齐，不再被挤压截断

  - 首次完成向导进入主页时弹出「加入 LuaFabric 官方交流群」弹窗（交流群图片 + 加入 / 不了按钮，仅弹一次），通过积极按钮加入跳转官方交流群

  - 赞助弹窗点击空白区域不再关闭

- 设置-主体与外观：「动态取色」开关默认改为开启

- 关于-官方社区：新增「开源地址」卡片（GitHub 图标，点击在默认浏览器打开 https://github.com/TheMuLing/LuaFabric_Studio）

- 设置-编辑器设置：新增「三方控件支持」开关（默认开启，分析 根目录/libs/*.dex 内控件，使布局助手支持预览携带三方控件的布局文件）

- 项目属性：调试模式卡片下新增「构建选项」卡片（android-studio 图标），「让 <软件名> 加密项目」开关默认开启，真实控制构建时的 lua 文件加密步骤

- AI 修复：

  - 修复添加提供商后崩溃：release 开启 R8 混淆剥离字段泛型签名（Signature），Gson 将 `List<ApiProvider>` 按 raw List 反序列化产出 LinkedTreeMap，渲染访问模型字段时 ClassCastException——新增 proguard keep 规则保留 AI Gson 模型类 + 配置加载后 `filterIsInstance` 类型消毒，双重修复

  - 修复添加 / 编辑提供商弹窗内四个编辑框在竖屏平板宽度不填满弹窗的问题（模型 ID 编辑框不覆盖右侧按钮）

- AI 侧边栏：

  - 对话记录按项目隔离存储（`ai_chat_history/<项目目录名>/`），各项目互不互通；删除项目时连带删除其全部对话记录；入口文案「历史记录」改为「对话记录」

  - AI 设置-技能：默认技能按 title 补齐合并，修复旧配置仅含 luafabric-studio 时缺失默认技能的问题

  - AI 设置-「API 提供商 / 技能 / 记忆」折叠菜单的展开 / 收起状态持久化，重启后保留

  - 对话气泡圆角跟随「设置-主体与外观」的形状圆角（4 / 8 / 12 / 16dp）

  - 修复长按用户消息时选择手柄与气泡同色（primary）导致不可见的问题

  - 选区引用给 AI：对话框以两行卡片显示——首行 format-quote-open 图标 + 项目相对路径：第 x~y 行 + 折叠箭头（可展开 / 收起），次行选区文本节选（换行 / tab 归并为单空格，超长截断加「...」）

  - 编辑器长按菜单「引用选区给 AI」图标改为 MD3 format-quote-close

  - AI 对话页右侧两个按钮由右下角移至右侧居中

- 控制台-输出页：复制选项弹窗预览区支持长按选区复制

- 手册：分类 tabs 移至搜索框上方并改为 TabLayout 同款样式（文字 + primary 下划线指示器，超宽可横向滚动）；搜索框圆角跟随主题形状圆角设置

## 26.09.19

- 构建修复：推断构建过程中断导致后续所有构建失败的问题——缓存 APK 解包改为原子写入并校验结果合法性，损坏缓存自动丢弃、下一次重建（`AndroidManifest.xml` 类错误由此消除）

- 调试运行：`import.lua` 运行时解包落盘到模块搜索目录（`app_lua`），`require "import"` 恢复可用

- 赞助：侧边栏赞助页点「投喂我」可跳过本轮赞助弹窗，多次点击仅计一次（幂等），避免反复打扰

- 向导权限精简：

  - 移除「相册访问权限」（Android 13+ 的 `READ_MEDIA_IMAGES`）条目

  - 移除「应用列表权限」（`APP_LIST_QUERY`，行为探测已安装应用数）条目

  - 保留「存储访问」与「网络」权限条目

- 项目图标选择：创建项目 / 项目属性页点击大图选图改用内置文件选择器，默认起始目录 `/sdcard/DCIM`，支持 jpg / jpeg / png / webp / gif / bmp；选中后在项目属性页复制到项目根目录（保持原文件名与净化）

- 控制台：新增侧栏面板（侧入 / 侧出动画），引入 socket 相关 Lua 运行时库（`socket` / `ltn12` / `mime`）

- 编辑器：右上角「运行」按钮改绿色；长按后，若当前标签为 `.lua` 文件则弹出菜单「运行当前文件」（Material Bug 图标），点击即仅运行当前文件、保留本文件内 `require` 与跳转；非 `.lua` 文件长按无响应；版本号升级 `26.09.19-gamma`

## 26.09.16

- 版本号更新：`versionCode 260901601` / `versionName 26.09.16-alpha`

- 启动权限引导：

  - 向导权限页新增「相册访问权限」条目（Android 13+ 的 `READ_MEDIA_IMAGES`，运行时权限单授权），权限卡片容器改为 LazyColumn 顺滑滚动

- 项目属性：

  - 点击属性页图标从系统相册（PhotoPicker）选取图片并复制到项目根目录，文件名净化、非法名回退 `icon.png`，路径写入 `settings.json` 持久化

  - 「图标路径」编辑框 FolderOpen 按钮调起项目内文件树，仅修改路径引用不复制文件

  - 图标预览与主页项目列表图标按 `settings.json` 的 `iconPath` 加载（越界/绝对路径净化），随文件 mtime 指纹自动刷新

- 控制台：

  - 新增捕获开关：`print()` / Toast / Snackbar 内容分别门控（默认 print、Toast 开，Snackbar 关），Toast/Snackbar 在 `make` 时登记实例与文本、`show()` 时输出，切换文件时补发未调用 `show()` 的实例

  - 输出页置顶固定「当前文件」标题行（长路径 MIDDLE 省略）；条目间细分割线（末条隐藏）；多选操作栏动作改为「取消」；页签状态跨面板重开保留、退出调试复位

  - 事件流条目：函数名以独立药丸 chip 展示（如 `(onPause)` + 「事件监听触发」），元数据行展示项目相对文件路径

  - 「仅事件」切换改为可选中 filter chip，选中取色与操作按钮一致

  - 数组真实内容解码（`byte[]` / `Byte[]` → `java.lang.Byte[]`，字节数组内容按 UTF-8 解码为可读字符串，解码失败回退元素列表）

  - 长内容在列表内以 `…` 截断展示，复制时不受截断限制（保持完整内容）

  - 复制选项弹窗：预览区实时展示将复制的内容，5 项勾选（Lua 层类型 / 真实类型 / 文件路径 / 时间 / 线程）默认全不勾、勾选即刷新预览，配色跟随莫奈取色

  - 清空确认弹窗：MD3 三操作（全部删除 / 取消 / 仅当前文件），按钮取色跟随主题

  - 修复：关闭「拦截界面跳转/结束请求」后重进项目（或切换其它项目）仍执行拦截——拦截总开关每次会话开始时从持久化设置重新同步
...
- newActivity 拦截（F3）重写：单会话单弹窗 + 折叠卡片列表

  - 同一会话内多次 `activity.newActivity` 调用汇入同一弹窗，同签名重复请求自动丢弃；须先单选一条，点「允许跳转」放行该条，其余取消

  - 折叠卡片：圆角 12dp、间距 4dp；左侧为相对项目根目录的文件路径，右侧为 chevron 折叠指示图标（请求未携带参数时不显示图标）；默认全部折叠，点图标折叠 / 展开同样计为选中

  - 携带参数：「携带参数：」独占头行，参数按「参数名 + Lua 类型」键值对排版（自动命名参数不显示名），条目间以不贴两侧的细分割线分隔；展开区背景左右收窄，避免遮盖卡片圆角

  - 选中高亮：点卡片非图标区域即选中，背景 + 主色描边突出；弹窗背景与「允许 / 取消」文本颜色跟随应用外观主题

- 修复：

  - 控制台输出归属错乱：新会话开启时清空全局缓冲；旧会话按项目 SHA-256 哈希归档（`sessions/<项目哈希>_<时间戳>/outputs.txt`），Logcat 日志写入项目独立目录，互不串台

  - Lua 运行时错误未汇入 F1 输出缓冲（type=error）：接线桥接层 `onError` 入口

  - 拦截方法名比对失败（cacheKey 带类前缀）：抽取裸方法名后再与 `newActivity` 比对

  - 返回旧界面浮球需点两次 / 旧浮窗残留：宿主销毁时切换至存活成员页，页面恢复时同步宿主引用
...
- 构建环境升级：
  ...
  - Gradle 9.1.0 → 9.5.0（AGP 8 需要 Gradle ≤ 9.5）

  - CMake 3.31.6 → 4.4.3

  - Java 17 → 21（app / core-apk / signer 的 `sourceCompatibility` 与 `jvmTarget`）

  - `minSdk` 24 / 23 → 29

  - `gradle.properties` 固定 JDK 21 构建（AGP 8 不支持 JDK ≥ 26）

- 修复项目属性页长按查看文档时闪退：注册 `MarkwonInlineParserPlugin`（新增 `markwon-inline-parser` 依赖），表格 / 删除线 / LaTeX 插件可正常使用

- 文档弹窗内容增加 12dp 内边距，并支持长按选区复制

- 新增 `require "yyjson"` 高性能 JSON 模块（自包含 C 绑定，基于 [yyjson](https://github.com/ibireme/yyjson)），提供 `read` / `decode` / `encode` / JSON Pointer / 读改写文档等接口
...
| 符号    | 等价于/描述     | 说明                                      | <br />         | <br />   |
| ----- | ---------- | --------------------------------------- | -------------- | -------- |
| `? :` | if-else 三元 | 条件运算符 `(cond) ? true_expr : false_expr` | <br />         | <br />   |
| `?.`  | 安全访问       | 可选链操作符，避免 nil 报错                        | <br />         | <br />   |
| `??`  | 空值合并       | 左侧为 nil 时返回右侧值                          | <br />         | <br />   |
| \`    | >\`        | 管道操作符                                   | 将值传入函数 \`value | > func\` |
| `!`   | `not`      | 逻辑非                                     | <br />         | <br />   |
| `!=`  | `~=`       | 不等于                                     | <br />         | <br />   |
| `&&`  | `and`      | 逻辑与                                     | <br />         | <br />   |
| \`    | <br />     | \`                                      | `or`           | 逻辑或      |
| `$`   | `local`    | 局部变量声明缩写                                | <br />         | <br />   |
| `@`   | `::`       | 标签声明（用于 goto）                           | <br />         | <br />   |
...
| `^=`  | `a ^= 2`       | `a = a ^ 2`          | <br />   | <br />          | <br /> |
| `..=` | `s ..= "x"`    | `s = s .. "x"`       | <br />   | <br />          | <br /> |
| `&=`  | `bits &= mask` | `bits = bits & mask` | <br />   | <br />          | <br /> |
...
| 特性类别   | 具体特性              | 语法示例                                             | <br />      | <br />    |
| ------ | ----------------- | ------------------------------------------------ | ----------- | --------- |
| 条件表达式  | 三元运算符             | `(a>b) ? a : b`                                  | <br />      | <br />    |
| <br /> | 空值合并              | `value ?? default`                               | <br />      | <br />    |
| <br /> | 可选链               | `obj?.field?.[index]`                            | <br />      | <br />    |
| 赋值操作   | 复合赋值              | \`+= -= \*= /= //= %= ^= ..= &=                  | = <<= >>=\` | <br />    |
| 逻辑操作   | 逻辑非               | `!condition`                                     | <br />      | <br />    |
| <br /> | 不等于               | `a != b`                                         | <br />      | <br />    |
| <br /> | 逻辑与               | `a && b`                                         | <br />      | <br />    |
| <br /> | 逻辑或               | \`a                                              | <br />      | b\`       |
| 函数式编程  | Lambda            | `\x,y -> x+y`                                    | <br />      | <br />    |
| <br /> | 管道                | \`value                                          | > func1     | > func2\` |
| 异常处理   | Try-Catch-Finally | `try ... catch(e) ... finally ... end`           | <br />      | <br />    |
| 控制流    | Switch-Case       | `switch x do case v: ... default ... end`        | <br />      | <br />    |
| <br /> | When              | `when c1 then ... case c2 then ... else ... end` | <br />      | <br />    |
| <br /> | Continue          | `continue`                                       | <br />      | <br />    |
| 资源管理   | Defer             | `defer cleanup() end`                            | <br />      | <br />    |
| 语法糖    | 局部声明              | `$ var = value`                                  | <br />      | <br />    |
| <br /> | 标签                | `@label@`                                        | <br />      | <br />    |
